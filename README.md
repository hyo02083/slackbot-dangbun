# slackbot-dangbun

당번을 자동으로 알려주는 Slack 봇입니다.

매번 순서를 수동으로 확인하거나 구두로 전달하다 보면 헷갈리고 깜빡하기 쉽습니다. 이 봇은 매월 1일 담당자를 자동 태그하고, `/당번` 명령어로 언제든 순서를 확인할 수 있어 번거로움을 줄여줍니다.

## 기능

- **월간 자동 알림** — 매월 1일 10:00에 해당 월 당번을 태그하여 채널에 공지
- **슬래시 커맨드로 관리** — 채널에서 직접 당번 순서 조회/추가/삭제/변경 가능
- **멤버 추가 시 자동 순서 배정** — 새로 추가된 멤버는 자동으로 가장 마지막 순서에 배정
- **멀티채널 지원** — 여러 채널에서 각각 독립적인 당번 순서를 관리

## 슬래시 커맨드

| 커맨드 | 설명 |
|--------|------|
| `/당번` | 전체 당번 순서 + 사용법 확인 |
| `/당번 추가 이름 U슬랙ID [순서], ...` | 멤버 추가 (여러 명 가능, `[순서]`는 선택값 — 생략 시 맨 뒤에 추가) |
| `/당번 삭제 U슬랙ID, ...` | 멤버 삭제 (여러 명 가능, `/당번 상세`에서 ID 확인) |
| `/당번 순서변경 이름1,이름2,...` | 순서 재배치 |
| `/당번 상세` | 전체 순서 + 슬랙ID 확인 |

## 설치 및 실행

### 1. Slack App 설정

1. [Slack API](https://api.slack.com/apps)에서 앱 생성
2. **Socket Mode** 활성화 → App-Level Token(`xapp-...`) 생성
3. **OAuth & Permissions** → Bot Token Scopes에 `chat:write`, `commands` 추가
4. **Slash Commands** → `/당번` 커맨드 등록
5. 워크스페이스에 앱 설치 → Bot Token(`xoxb-...`) 복사
6. 메시지를 보낼 채널에 봇 초대

### 2. 환경 설정

```bash
cp .env.example .env
```

`.env` 파일에 토큰을 입력합니다.

```
SLACK_BOT_TOKEN=xoxb-...
SLACK_APP_TOKEN=xapp-...
```

### 3. 당번 명단 설정

봇이 처음 실행된 후 채널에서 `/당번 추가` 명령어로 멤버를 추가하면 됩니다. 여러 명을 한 번에 추가할 수도 있습니다.

```
/당번 추가 홍길동 U12345678, 김철수 U87654321
```

> Slack User ID는 프로필 → ··· → **멤버 ID 복사**에서 확인할 수 있습니다.

### 4. 실행

```bash
pip install -r requirements.txt
python bot.py
```

## 데이터 구조

채널별로 독립적인 데이터가 `data/` 폴더에 자동 생성됩니다.

```
data/
  C0AK2GMBH36/        ← A채널
    leaders.json       ← 당번 명단
    config.json        ← 시작 연월 설정
  C0BK3HNCJ47/        ← B채널
    leaders.json
    config.json
```

- `leaders.json` — 당번 명단 (`leaders.example.json` 참고)
- `config.json` — 당번 계산의 시작 기준점 (`config.example.json` 참고)
- 두 파일 모두 채널에서 처음 `/당번` 명령어를 사용하면 자동으로 생성됩니다.
